...
final class Admin extends Controller {
	 ...
	 * @return WP_Rest_Response
	 */
	public function render_tokens( $request ) {
		...
		// Get email event.
		$email_event = sanitize_text_field( $request->get_param( 'hp_event' ) );
		...
		return hp\rest_response( 200,
			[
				'html' => '<p data-block="' . $block . '">' . $tokens . '</p>'
			]
		);
	}
}
