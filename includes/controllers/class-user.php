...
class User extends Controller {
	...
	public function register_user( $request ) {
		...
		if ( $form->get_value( 'username' ) ) {
			if ( sanitize_user( $form->get_value( 'username' ), true ) !== $form->get_value( 'username' ) ) {
				return rest_error( 400, esc_html__( 'Username contains invalid characters', 'hivepress' ) );
			} elseif ( username_exists( $form->get_value( 'username' ) ) ) {
				return rest_error( 400, esc_html__( 'This username is already in use', 'hivepress' ) );
			}
		}

		// Check email.
		if ( email_exists( $form->get_value( 'email' ) ) ) {
			return rest_error( 400, esc_html__( 'This email is already registered', 'hivepress' ) );
		}
		...
		if ( ! $user->save() ) {
			return rest_error( 400, esc_html__( 'Error registering user', 'hivepress' ) );
		}
	...
	public function login_user( $request ) {
		...
		if ( false === $user ) {
			return rest_error( 401, esc_html__( 'Username or password is incorrect', 'hivepress' ) );
		}
	...
	public function request_password( $request ) {
		...
		if ( false === $user ) {
			if ( is_email( $form->get_value( 'username_or_email' ) ) ) {
				return rest_error( 404, esc_html__( "User with this email doesn't exist", 'hivepress' ) );
			} else {
				return rest_error( 404, esc_html__( "User with this username doesn't exist", 'hivepress' ) );
			}
		}
	...
	public function reset_password( $request ) {
		...
		if ( is_wp_error( $user ) ) {
			return rest_error( 401, esc_html__( 'Password reset key is expired or invalid', 'hivepress' ) );
		}
	...
	public function update_user( $request ) {
		...
		if ( get_current_user_id() === $user->get_id() && ( $form->get_value( 'email' ) !== $user->get_email() || $form->get_value( 'password' ) ) ) {
			if ( $form->get_value( 'current_password' ) === null ) {
				return rest_error( 400, esc_html__( 'Current password is required', 'hivepress' ) );
			}

			if ( ! wp_check_password( $form->get_value( 'current_password' ), $user->get_password(), $user->get_id() ) ) {
				return rest_error( 401, esc_html__( 'Current password is incorrect', 'hivepress' ) );
			}
		}
		...
		if ( ! $user->save() ) {
			return rest_error( 400, esc_html__( 'Error updating user', 'hivepress' ) );
		}
	...
	public function delete_user( $request ) {
		...
		if ( get_current_user_id() === $user->get_id() ) {
			...
			if ( ! wp_check_password( $form->get_value( 'password' ), $user->get_password(), $user->get_id() ) ) {
				return rest_error( 401, esc_html__( 'Password is incorrect', 'hivepress' ) );
			}
		}

		// Delete user.
		if ( ! $user->delete() ) {
			return rest_error( 400, esc_html__( 'Error deleting user', 'hivepress' ) );
		}
