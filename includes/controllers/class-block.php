...
class Block extends Controller {
	...
	public function get_block( $request ) {
		...
		$nonce = hp_get_array_value( $request->get_params(), '_wpnonce', $request->get_header( 'X-WP-Nonce' ) );
		...
		// Get template.
		$template_args = hp_get_array_value( hivepress()->get_config( 'templates' ), str_replace( '-', '_', $request->get_param( 'template_name' ) ) );
		...
		// Get block.
		$block_args = hp_search_array_value( $template_args, [ 'blocks', str_replace( '-', '_', $request->get_param( 'block_name' ) ) ] );
